...
const categories = [
  { name: 'Favorites', icon: Star, count: nodeTypes.filter(n => n.isFavorite).length },
  { name: 'Input/Output', icon: PenLine, count: nodeTypes.filter(n => n.category === 'Input/Output').length },
  { name: 'AI & ML', icon: Bot, count: nodeTypes.filter(n => n.category === 'AI & ML').length },
  { name: 'Logic', icon: Code, count: nodeTypes.filter(n => n.category === 'Logic').length },
  { name: 'Database', icon: Database, count: nodeTypes.filter(n => n.category === 'Database').length },
  { name: 'Integration', icon: Zap, count: nodeTypes.filter(n => n.category === 'Integration').length },
];
                ...
                <AccordionContent className="space-y-2 pt-2">
                  {categoryNodes.map((node) => (
                    <div
                      key={node.id}
                      className={cn(
                        "group relative p-3 rounded-lg border border-border bg-card hover:bg-accent/50 cursor-grab transition-colors",
                        "hover:shadow-sm"
                      )}
                      draggable
